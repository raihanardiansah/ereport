<x-layouts.app title="Menunggu Pembayaran">
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-4xl mx-auto px-4">
            ...
            <div id="waiting-card" class="bg-white rounded-2xl shadow-sm p-8 mb-6 text-center">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-xl font-semibold text-gray-900" id="expiry-time">
                        {{ $transaction->expiry_time ? $transaction->expiry_time->format('d M Y, H:i') . ' WIB' : '24 jam dari sekarang' }}
                    </p>
                </div>
            </div>
            ...
            <div id="payment-content" class="{{ $wrapperClass }} hidden">
                <!-- Detail Pembayaran -->
                <div class="bg-white rounded-2xl shadow-sm p-6 {{ $isQris ? 'order-2' : '' }}">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            ...
                            <span class="font-mono font-semibold text-gray-900">{{ $transaction->order_id }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-gray-600">Paket</span>
                            <span class="font-semibold text-gray-900">{{ $transaction->package->name }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-gray-600">Durasi</span>
                            <span class="font-semibold text-gray-900">{{ $transaction->package->duration_months }} bulan</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="text-gray-600">Metode</span>
                            <span class="font-semibold text-gray-900">{{ $transaction->payment_method ? ucwords(str_replace('_', ' ', $transaction->payment_method)) : 'Virtual Account' }}</span>
                        </div>
                        <div class="flex justify-between py-3">
                            <span class="text-gray-900 font-semibold">Total Bayar</span>
                            <span class="text-2xl font-bold text-primary-600">Rp {{ number_format($transaction->gross_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>

                <!-- Instruksi Pembayaran -->
                <div class="bg-white rounded-2xl shadow-sm p-6 {{ $isQris ? 'order-1' : '' }}">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4">Instruksi Pembayaran</h2>
                    
                    @if(($transaction->payment_method === 'gopay' || $transaction->payment_method === 'qris') && $transaction->qr_code_url)
                        ...
                        <div class="grid md:grid-cols-2 gap-8">
                            ...
                            <div>
                                ...
                                <div class="bg-white rounded-xl p-4 shadow-sm mb-4">
                                    <div class="text-center md:text-left">
                                        <p class="text-sm font-semibold text-gray-700 mb-1">{{ auth()->user()->school->name ?? 'E-Report' }}</p>
                                        @if(isset($transaction->merchant_id))
                                        <p class="text-xs text-gray-500 mb-2">NMID: {{ $transaction->merchant_id }}</p>
                                        @endif
                                ...
                                <div class="bg-white rounded-xl p-4 shadow-sm mb-4">
                                    ...
                                    <p class="text-3xl font-bold text-indigo-600">Rp {{ number_format($transaction->gross_amount, 0, ',', '.') }}</p>
                                </div>
                    ...
                    @elseif($transaction->payment_method === 'gopay' || $transaction->payment_method === 'qris')
                    <!-- QRIS Loading State -->
                    ...
                    <div class="bg-primary-50 border border-primary-200 rounded-xl p-4 mb-4">
                        ...
                        <div class="flex items-center justify-between bg-white rounded-lg p-3 gap-3">
                            <span class="text-lg md:text-xl font-bold text-primary-600 font-mono tracking-wide break-all" id="va-number">{{ $transaction->va_number }}</span>
                        ...
                        @endphp
                        <p class="text-sm text-gray-600 mt-2">Bank {{ $bankName }} Virtual Account</p>
                    </div>
            ...
            <div id="action-buttons" class="mt-6 flex flex-col sm:flex-row gap-4">
                <a href="{{ route('dashboard') }}" class="flex-1 bg-white border border-gray-300 text-gray-700 font-semibold py-3 px-6 rounded-xl hover:bg-gray-50 transition-colors text-center">
    ...
    <script>
        ...
        async function cancelTransaction() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '{{ route("subscriptions.history") }}';
                    }, 1500);
                } else {
        ...
        @if($transaction->payment_method === 'gopay' && $transaction->expiry_time)
