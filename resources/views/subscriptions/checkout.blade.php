...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('subscriptions.packages') }}" class="text-indigo-600 hover:text-indigo-800 flex items-center text-sm">
    ...
    <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="card">
                ...
                <form action="{{ route('subscriptions.process', $package) }}" method="POST" id="checkout-form">
                    @csrf
                    <input type="hidden" name="promo_code" id="promo-code-input" value="{{ $promoCode ?? '' }}">
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-xl p-5 mb-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 text-sm mb-2">{{ $package->description ?? 'Paket langganan' }}</p>
                                <div class="flex flex-wrap gap-3 text-sm text-gray-600 dark:text-gray-400">
                                    <span>👥 {{ $package->max_users }} pengguna</span>
                                    <span>📝 {{ $package->max_reports_per_month }} laporan/bulan</span>
                                    <span>📅 {{ $package->duration_months }} bulan</span>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $package->formatted_price }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex gap-3">
                            <input type="text" id="promo-input" value="{{ $promoCode ?? '' }}"
                            ...
                            <button type="button" id="apply-promo-btn" onclick="validatePromo()" 
                                class="px-6 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 font-medium {{ $promoCode ? 'hidden' : '' }}">
                                Terapkan
                            </button>
                            <button type="button" onclick="removePromo()" class="px-6 py-3 text-red-600 hover:text-red-800 font-medium {{ $promoCode ? '' : 'hidden' }}" id="remove-promo-btn">
                                Hapus
                            </button>
                        </div>
                        <div id="promo-message" class="mt-2 text-sm {{ $promoCode ? 'text-green-600' : 'hidden' }}">
                            @if($appliedPromo)
                                ✓ Promo "{{ $appliedPromo->name }}" ({{ $appliedPromo->formatted_value }}) berhasil diterapkan!
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 mb-6">
                        ...
                        <p class="text-gray-900 dark:text-white font-medium">{{ $school->name }}</p>
                        <p class="text-gray-500 dark:text-gray-400 text-sm">{{ $school->email }}</p>
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="card sticky top-24">
                ...
                <div class="space-y-3 mb-4">
                    ...
                    <div id="discount-row" class="flex justify-between text-green-600 {{ $promoDiscount > 0 ? '' : 'hidden' }}">
                        <span>Diskon Promo</span>
                        <span id="discount-amount">-Rp {{ number_format($promoDiscount, 0, ',', '.') }}</span>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                    <div class="flex justify-between">
                        ...
                        <span id="total-price" class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">Rp {{ number_format($finalPrice, 0, ',', '.') }}</span>
                    </div>
                </div>
...
async function validatePromo() {
    ...
    try {
        const response = await fetch('{{ route("subscriptions.validate-promo") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
function removePromo() {
    ...
    document.getElementById('total-price').textContent = 'Rp {{ number_format($package->price, 0, ",", ".") }}';
}
</script>
@endsection
