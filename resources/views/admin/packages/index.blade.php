...
@section('title', 'Kelola Paket & Promosi')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="card">
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['total_packages'] }}</p>
        ...
        <div class="card">
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['active_packages'] }}</p>
        ...
        <div class="card">
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['total_promotions'] }}</p>
        ...
        <div class="card">
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['active_promotions'] }}</p>
    ...
    <div class="card">
        ...
        @if($packages->count() > 0)
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4" id="packages-grid">
                @foreach($packages as $package)
                    <div class="relative border rounded-xl p-5 {{ $package->is_active ? 'border-gray-200 dark:border-gray-700' : 'border-gray-100 dark:border-gray-800 opacity-60' }} {{ $package->is_featured ? 'ring-2 ring-indigo-500' : '' }}">
                        @if($package->badge_text)
                            <span class="absolute -top-3 left-4 px-3 py-1 text-xs font-semibold rounded-full {{ $package->badge_color ?? 'bg-indigo-500 text-white' }}">
                                {{ $package->badge_text }}
                            </span>
                        @endif
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-indigo-600 dark:text-indigo-400 mt-1">{{ $package->formatted_price }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $package->duration_months }} bulan</p>
                            </div>
                            <div class="flex items-center space-x-1">
                                <button onclick="togglePackage({{ $package->id }})" title="{{ $package->is_active ? 'Nonaktifkan' : 'Aktifkan' }}"
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">{{ $package->description ?? 'Tidak ada deskripsi' }}</p>

                        <div class="text-xs text-gray-500 dark:text-gray-400 space-y-1 mb-4">
                            <p>👥 Max {{ $package->max_users }} pengguna</p>
                            <p>📝 Max {{ $package->max_reports_per_month }} laporan/bulan</p>
                        </div>

                        <div class="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                            <span class="text-xs {{ $package->is_active ? 'text-green-600' : 'text-gray-400' }}">
                                {{ $package->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                            <div class="flex space-x-2">
                                <a href="{{ route('admin.packages.edit', $package) }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Edit</a>
                                <button onclick="deletePackage({{ $package->id }})" class="text-red-600 hover:text-red-800 text-sm font-medium">Hapus</button>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        @else
            <div class="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p>Belum ada paket. <a href="{{ route('admin.packages.create') }}" class="text-indigo-600 hover:underline">Buat paket pertama</a></p>
            </div>
        @endif
    </div>
    ...
    <div class="card">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.promotions.create') }}" class="btn-primary text-sm">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                Tambah Promosi
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @foreach($promotions as $promo)
                            <tr class="{{ $promo->is_active ? '' : 'opacity-50' }}">
                                <td class="py-4">
                                    <span class="font-mono font-semibold text-indigo-600 dark:text-indigo-400">{{ $promo->code }}</span>
                                </td>
                                <td class="py-4">
                                    <span class="text-gray-900 dark:text-white">{{ $promo->name }}</span>
                                </td>
                                <td class="py-4">
                                    <span class="font-medium text-green-600 dark:text-green-400">{{ $promo->formatted_value }}</span>
                                </td>
                                ...
                                <td class="py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $promo->status_color }}">
                                        {{ $promo->status }}
                                    </span>
                                </td>
                                <td class="py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        <button onclick="togglePromotion({{ $promo->id }})" title="Toggle Status"
                                        ...
                                        <a href="{{ route('admin.promotions.edit', $promo) }}" class="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
                                            <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <button onclick="deletePromotion({{ $promo->id }})" class="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
        ...
        @else
            <div class="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <p>Belum ada promosi. <a href="{{ route('admin.promotions.create') }}" class="text-indigo-600 hover:underline">Buat promosi pertama</a></p>
            </div>
        @endif
    </div>
</div>
...
async function syncPackages() {
    try {
        const response = await fetch('{{ route("admin.packages.sync") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json',
