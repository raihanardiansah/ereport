...
<div class="space-y-6">
    ...
    <!-- Filters -->
    <div class="card">
        <form action="{{ route('admin.messages') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <select name="source" class="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:bg-gray-800 dark:text-white">
                ...
                <option value="in_app" {{ request('source') == 'in_app' ? 'selected' : '' }}>Dalam Aplikasi</option>
            </select>
            ...
            @if(request()->hasAny(['search', 'status', 'source', 'channel']))
                <a href="{{ route('admin.messages') }}" class="btn-outline">Reset</a>
            @endif
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($messages as $message)
                        <tr class="{{ $message->isUnread() ? 'bg-primary-50 dark:bg-primary-900/10' : '' }} hover:bg-gray-50 dark:hover:bg-gray-800">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center">
                                        <span class="text-primary-600 dark:text-primary-400 font-semibold">{{ strtoupper(substr($message->name, 0, 1)) }}</span>
                                    </div>
                                    <div class="ml-4">
                                        ...
                                        @if($message->school_name)
                                            <div class="text-xs text-gray-400 dark:text-gray-500">{{ $message->school_name }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-white font-medium">
                                    {{ $message->subject ?: $message->type_label }}
                                </div>
                                <div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">
                                    {{ Str::limit($message->message, 60) }}
                                </div>
                                @if($message->source === 'in_app')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $message->channel_color }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.messages.show', $message) }}" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 dark:hover:text-primary-300">
                                    Lihat
                                </a>
                            </td>
                        </tr>
