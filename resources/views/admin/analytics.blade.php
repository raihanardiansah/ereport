<x-layouts.app title="Analytics Platform">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.export.csv', 'reports') }}" class="btn-secondary text-sm">
                <svg class="w-4 h-4 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Export CSV
            </a>
            <a href="{{ route('admin.export.pdf') }}" class="btn-primary text-sm">
                <svg class="w-4 h-4 mr-1 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-green-600 mt-1">{{ $stats['active_schools'] }} aktif</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($stats['total_reports']) }}</p>
                    <p class="text-xs text-blue-600 mt-1">{{ $stats['this_month_reports'] }} bulan ini</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($stats['total_cases']) }}</p>
                    <p class="text-xs text-orange-600 mt-1">{{ $stats['open_cases'] }} terbuka</p>
                </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="flex items-end justify-between h-48 gap-2">
                @php $maxCount = max(array_column($reportsTrend, 'count')) ?: 1; @endphp
                @foreach($reportsTrend as $month)
                <div class="flex-1 flex flex-col items-center">
                    <div class="w-full bg-primary-100 dark:bg-primary-900/30 rounded-t" 
                        ...
                        <div class="w-full h-full bg-primary-500 rounded-t hover:bg-primary-600 transition-colors" 
                             title="{{ $month['count'] }} laporan"></div>
                    </div>
                    <span class="text-xs text-gray-500 dark:text-gray-400 mt-2">{{ $month['label'] }}</span>
                    <span class="text-xs font-semibold text-gray-700 dark:text-gray-300">{{ $month['count'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
                        <div class="bg-primary-500 h-2 rounded-full" style="width: {{ ($count / $totalCat) * 100 }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('admin.school.detail', $school) }}" class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    <div class="flex items-center">
                        <div class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center mr-3">
                            <span class="text-primary-700 font-bold">{{ strtoupper(substr($school->name, 0, 1)) }}</span>
                        </div>
                        <div>
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $school->name }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $school->users_count }} user</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="font-semibold text-gray-900 dark:text-gray-100">{{ $school->reports_count }}</p>
            ...
            <div class="space-y-3">
                ...
                <div class="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <p class="font-medium text-gray-900 dark:text-gray-100 text-sm">{{ Str::limit($report->title, 40) }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                {{ $report->school->name ?? 'Unknown' }} • {{ $report->created_at->diffForHumans() }}
                            </p>
                        </div>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('admin.export.csv', 'schools') }}" class="btn-secondary text-sm">
                📊 Export Sekolah (CSV)
            </a>
            <a href="{{ route('admin.export.csv', 'reports') }}" class="btn-secondary text-sm">
                📋 Export Laporan (CSV)
