<x-layouts.app title="Profil {{ $student->name }}">
    <div class="mb-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <div class="w-20 h-20 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center mr-6">
                    <span class="text-white text-3xl font-bold">{{ strtoupper(substr($student->name, 0, 1)) }}</span>
                </div>
                <div class="flex-1">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $student->name }}</h1>
                    <p class="text-gray-500 dark:text-gray-400">{{ $student->username }} • {{ $student->email }}</p>
                    <div class="flex items-center gap-4 mt-2 text-sm">
                        <span class="text-gray-600 dark:text-gray-400">NISN: {{ $student->nip_nisn ?? '-' }}</span>
                        <span class="text-gray-600 dark:text-gray-400">{{ $student->phone ?? '-' }}</span>
                    </div>
                </div>
            </div>
            <a href="{{ route('pdf.student', $student) }}" class="btn-primary inline-flex items-center">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('student-cases.show', $case) }}" class="block p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3 max-h-96 overflow-y-auto">
                ...
                <a href="{{ route('reports.show', $report) }}" class="block p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    ...
                    <p class="text-xs text-gray-500 mt-1">{{ ucfirst($report->category) }} • {{ ucfirst($report->status) }}</p>
                </a>
                @endforeach
            </div>
            @else
